...
import {allIdsByIdObjectStrictNumbered} from "../source/types";

it ("should create a new element if none existed previously", () => {
    const theObject: {allIdsById: allIdsByIdObjectStrictNumbered} = {allIdsById: {allIds: [], byId: {}}};
...
it ("should overwrite only one element in the object with the new value", () => {
    const theObject: {allIdsById: allIdsByIdObjectStrictNumbered} = {allIdsById: {allIds: [123], byId: {123: {test: true, untouched: false, thenull: null}}}};
    theObject.allIdsById = addToIdBasedObjectOrMerge(theObject.allIdsById, 123, {test: "string"});
...
it ("should handle null elements and null previously explicitely undefined items", () => {
    const theObject: {allIdsById: allIdsByIdObjectStrictNumbered} = {allIdsById: {allIds: [123], byId: {123: {test: undefined, untouched: false, thenull: null}}}};
...
it ("should handle null elements and null previously undefined items", () => {
    const theObject: {allIdsById: allIdsByIdObjectStrictNumbered} = {allIdsById: {allIds: [123], byId: {123: {untouched: false, thenull: null}}}};
    theObject.allIdsById = addToIdBasedObjectOrMerge(theObject.allIdsById, 123, {test: null});
    expect(theObject.allIdsById).toEqual({allIds: [123], byId: {123: {test: null, untouched: false, thenull: null}}});
});
...
it ("cannot handle objects without the allIds byId structure", () => {
    let theObject = {};
    const theTest = () => { theObject = addToIdBasedObjectOrMerge((theObject as {allIdsById: allIdsByIdObjectStrictNumbered}).allIdsById, 123, {test: null});};
